...
| ID | Title | St | Sev | Mechanism | Gate |
| --- | --- | --- | --- | --- | --- |
| SEC-014 | No default recalc time budget | Open | P2 | docs/21's calc table lists "recalc time budget (cancellable)" as a limit. SEC-012 built the cancellation half and made it reachable, but nothing sets a *default* deadline, so an embedder who passes no token still gets an unbounded recalc. Needs a host-supplied clock: `Instant::now` panics on `wasm32-unknown-unknown`, which is why the token is a predicate rather than a deadline. | A recalc with no token stops itself after the budget |
| SEC-013 | No bound on snapshot size | Open | P2 | The last row of docs/21's scale table. `Workbook::from_snapshot` accepts any length; over the wire the collaboration server's `max_message_bytes` bounds it in practice, but the model entry point itself does not, so a host calling it directly has no ceiling. | An oversized snapshot is refused, not decoded |
