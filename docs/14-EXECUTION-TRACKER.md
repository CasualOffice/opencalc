...
| SEC-003 | Docker secret hygiene | Open | P0 | `.env` sits in the build context and the compose default secret is a known string. | Image scan finds no secret; compose refuses the default |
| PROD-12 | Docker collaboration endpoint is unreachable | Open | P0 | `OPENCALC_COLLAB_WS` defaults to `ws://127.0.0.1`, which is the *browser's* loopback, and an HTTPS page cannot open `ws://`. | Two contexts through a non-loopback host both reach `live` |
...
| DEP-06 | No metrics | Open | P1 | `/stats` returns two integers. Saves, latencies, retries and lease changes are computed and discarded into log lines. | `/metrics` scrapes; an operator can alert on failed saves |
| DEP-07 | No published image, and CI never builds one | Open | P1 | Twelve CI jobs, no docker build — so nothing tests that the Dockerfiles still compile. | A tagged image is pushed and pulled |
