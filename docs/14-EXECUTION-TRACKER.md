...
| M8-4a | Editable font-size combobox (type any size) | Done | The font-size control was a fixed `<select>` (8–36 presets), so you couldn't type sizes like 13 or 22, and a cell with such a size showed blank. Replaced it with an editable `<input list=datalist>` combobox: type any size (clamped to Excel's 1–409 pt; blank/0 clears), keep the presets in the dropdown, Enter commits and returns focus to the grid, Esc reverts, focus selects-all. `refreshFormulaBar` already sets `.value` so arbitrary sizes now display. Live-verified: typed 22 (not a preset) → A2 renders at 22pt, row auto-grows, box shows 22. Grow/shrink buttons + searchable font menu are the remaining M8-4 half. |
