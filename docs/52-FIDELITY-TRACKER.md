...
| FID-10 | Inside borders: `horizontal`, `vertical` in `CT_Border` | P1 | ✅ | Modelled as `Borders::inside_horizontal` / `inside_vertical` and written in CT_Border's sequence order. No UI, and correctly so: OOXML uses these children only in differential formats, and the range placements a user reaches for (`inner`, `horizontal`, `vertical`) are per-cell edges, which the border picker already has. |
