...
| FN-04 | Logical and information | 8 | ✅ | TRUE, FALSE, N, TYPE, ERROR.TYPE, ISREF, ISFORMULA, SHEET, SHEETS. Three that are not what they look like: **`N` reads text as 0 rather than erroring**, which is the entire reason the function exists, so routing it through the ordinary numeric coercion would defeat it. **`ISREF` has to inspect the expression, not the value** — by the time a function receives an argument the evaluator has already resolved a reference to its contents, so asking the value answers FALSE for every reference. And **`ISFORMULA` of a non-reference is `#VALUE!`, not FALSE**, since FALSE would read as "that cell has no formula", a different claim. `ERROR.TYPE` numbers `#SPILL!` 9, which post-dates the 5th edition (that stops at 7) but is what a workbook round-tripped through Excel expects. The catalog/dispatch sync test caught SHEET and SHEETS listed with no implementation. 4 tests. 32.3% → 34.0% **CELL and INFO are in, and both decline what they cannot know.** `CELL` reports on the *reference*, not the value at it, so the argument stays an expression — evaluating it first leaves a number and loses the address. `CELL("filename")` and `INFO("directory")` return `#N/A`: there is no path here, and `""` would read as "an unsaved workbook", which is a different claim. `CELL("format")` reads the same number-format code the renderer draws with — `cell_number_format` was made public rather than resolved a second time, because two resolutions are two things that can disagree. |
| FN-03 | Date and time | 16 | ✅ | TIME, HOUR, MINUTE, SECOND, DAYS, DAYS360, DATEDIF, WEEKNUM, ISOWEEKNUM, YEARFRAC, NETWORKDAYS, WORKDAY. The traps: **time components must round before splitting** — 13:45:30 as a day fraction is not exactly representable, so truncating the raw product gives 29 seconds where the sheet shows 30; **DAYS360's US convention is deliberately asymmetric**, since only after clamping the start's 31st does the end's 31st move, and writing the two clamps symmetrically silently changes bond maths; **ISO weeks belong to the year containing their Thursday**, so 2021-01-01 is week 53 of 2020 while WEEKNUM says 1 — a naive day count gets this wrong every January; and DATEDIF reports **#NUM! for reversed dates** rather than a negative span. NETWORKDAYS counts inclusively at both ends and negates when reversed. YEARFRAC implements all five day-count bases, where a wrong one is close enough to look right and wrong enough to matter in interest. **Still open**: NOW, TODAY, DATEVALUE, TIMEVALUE and the `.INTL` variants — NOW/TODAY need a clock seam so tests stay deterministic, which is a design decision rather than a gap to fill blindly. 6 tests. 34.0% → 36.8% **The volatile and parsing gaps are closed**: TODAY, NOW, RAND, RANDBETWEEN, DATEVALUE, TIMEVALUE, NETWORKDAYS.INTL, WORKDAY.INTL. The design point is that **the engine reads no clock and no entropy of its own** — `Workbook::volatile_now` and `volatile_seed` are supplied by the host and excluded from serialisation, because they are environment rather than document state and an engine that reaches for the wall clock cannot be tested or replayed. A test sets both and asserts the same values twice, then a different seed and asserts they change. Two `RAND()` calls in one formula must differ, which a seed alone cannot do, so the evaluator counts its draws and mixes the counter in. Volatiles are flagged in the dependency graph like a defined name — they depend on nothing in the sheet and change anyway, so a dependency-driven recalculation would never reach them and `=TODAY()` would show yesterday. `DATEVALUE` **refuses** `03/04/2024`: it is 3 April in most of the world and 4 March in the United States, and with no locale to decide, guessing is wrong a third of the time. The `.INTL` weekend mask starts on **Monday** while `WEEKDAY` counts from Sunday — reading it with a Sunday origin shifts every weekend by a day. Coverage 85.1% → 86.8%. |
...
| FN-02 | Text | 17 | ✅ | CHAR, UNICHAR, CODE, UNICODE, CLEAN, T, FIXED, DOLLAR, NUMBERVALUE. **CHAR/CODE differ from their Unicode twins only by range**, so implementing them as one function would accept `CHAR(955)` and return λ, which Excel refuses — and `CODE("λ")` is `#VALUE!` rather than a code point, because CODE is byte-oriented. **FIXED's negative `decimals` rounds to the left of the point** (`FIXED(1234.5,-2)` is `1,200`); clamping it to zero reads as harmless and quietly gives `1,235`. **`T` does not convert** — a number gives empty text, which is the whole difference from TEXT. `NUMBERVALUE` takes its separators as arguments rather than inferring a locale, so the same text parses identically everywhere. **Still open**: the byte-oriented `*B` variants and the East Asian ASC/JIS/DBCS/PHONETIC/BAHTTEXT, which need real DBCS handling rather than a stub. 4 tests. 38.5% → 40.2% **The byte-oriented family is in** with real DBCS semantics: LENB, LEFTB, RIGHTB, MIDB, FINDB, SEARCHB, REPLACEB, plus ASC/JIS width conversion, BAHTTEXT and the legacy USDOLLAR alias. Aliasing the `*B` functions to their character twins — which is what they collapse to in a single-byte locale — would silently halve every count on Japanese or Chinese text, which is precisely the data they exist for. A cut landing inside a wide character yields a space for the half it cannot represent, as Excel does. Two tests: one on wide text, one asserting every pair agrees exactly on ASCII. BAHTTEXT handles the two Thai irregularities a digit-by-digit rendering gets wrong — a tens digit of one is `สิบ`, and a units digit of one after any tens is `เอ็ด`. |
| FN-08 | Statistical and database | 92 | ✅ | Descriptive statistics (AVEDEV, DEVSQ, GEOMEAN, HARMEAN, MODE, SKEW, KURT, VAR, VARP, PERCENTILE, QUARTILE, PERCENTRANK, TRIMMEAN, COUNTBLANK, STANDARDIZE), regression (CORREL, PEARSON, RSQ, COVAR, SLOPE, INTERCEPT, STEYX, FORECAST) and the first distributions (NORMDIST, NORMINV, NORMSDIST, NORMSINV, EXPONDIST, POISSON, BINOMDIST, FISHER, FISHERINV, GAMMALN). **The discrete distributions sum in log space** — `m^k/k!` and `C(n,k)` both overflow `f64` long before the probability does, so a direct implementation returns NaN for `POISSON(200,300,TRUE)` while the answer is an ordinary number. `SLOPE`/`INTERCEPT` take **y before x**, so a test uses a perfect line where an argument-order slip is unmissable. Mismatched paired ranges are `#N/A`, not zipped to the shorter one, which would silently answer over part of the data. `MODE` of all-distinct values errors rather than returning the first. **`erf` is exact at zero by construction**: the rational approximation gives ~1e-9 there, making `NORMSDIST(0)` read 0.5000000005 — a wart in the first place anyone checks. Two test expectations I wrote from memory were wrong (the sample's mean is 5.5556, not 5; `NORMSDIST(1.96)` is 0.97500210, not 0.975) and were recomputed — the "coverage is not correctness" rule applying to the tests as much as the code. **Still open**: the chi-square, t, F, beta and gamma families, which need incomplete-gamma/beta and their inverses. 7 tests. 40.2% → 49.4% **Chi-square / t / F / gamma / beta added.** All five reduce to the regularized incomplete gamma or beta, so those are implemented once rather than as five series. Each uses the standard **series/continued-fraction split**: the series converges quickly below the distribution's mean and stalls above it, the continued fraction does the opposite — using either alone is accurate over half its range and quietly wrong over the other half. `beta_i` likewise flips to `1 − I_(1−x)(b,a)` above the mode for the same reason. **`CHIDIST` and `FDIST` are the *upper* tail**, unlike almost every other `*DIST`; returning the CDF gives `1 −` the right answer, which is a plausible probability and wrong every time, so a test pins both at x=0. Inverses bisect over a bracket grown until it contains the root — a fixed bracket silently returns its own endpoint for extreme probabilities. Tested by round-tripping each pair *and* against known points (χ² with 2 df is exponential, t with huge df is normal, Beta(1,1) is uniform), since a shared error in the underlying gamma or beta would cancel in a round trip alone. 2 tests. 69.1% → 71.9% **Tests, SUBTOTAL and the bill/loan financials added.** `SUBTOTAL`'s 100-series excludes hidden rows and the 1-series includes them — that distinction *is* the function, since a filtered list must not report a total counting what is hidden. `CUMIPMT`/`CUMPRINC` sum the per-period figures rather than deriving independently, so they always agree with `PMT`; a test asserts principal + interest = total payments and principal repaid = the loan. `ISO.CEILING` and `ECMA.CEILING` agree on positives and differ on negatives, which is the only reason both names exist. `ROMAN` refuses a non-zero form rather than silently answering in the classic one. **A test of mine was wrong, not the code**: my paired t-test sample differed by a constant, so the differences had zero variance and the statistic is genuinely undefined — `#DIV/0!` was right and the data was bad. 5 tests. 71.9% → 77.5% **The database family is in**: DSUM, DAVERAGE, DCOUNT, DCOUNTA, DGET, DMAX, DMIN, DPRODUCT, DSTDEV, DSTDEVP, DVAR, DVARP — twelve functions on one criteria engine, because writing them separately is how twelve copies of the same rules drift apart. The rules are the part worth stating: the criteria block's first row names fields, **conditions across a row are AND and rows are OR**, and an empty criteria cell is not a condition — reading it as "equals blank" would exclude every row. `field` may be a name, a 1-based index or a reference, since a file written by someone else will use whichever they preferred. `DGET` returns `#VALUE!` for no match and `#NUM!` for several rather than returning the first, which would be a plausible wrong answer. Coverage 77.5% → 80.9%. |
| FN-07 | Financial | 53 | ✅ | The annuity family (PV, FV, PMT, NPER, RATE, IPMT, PPMT, ISPMT), cash-flow analysis (NPV, IRR, MIRR, XNPV, XIRR, FVSCHEDULE), depreciation (SLN, SYD, DB, DDB) and the rate conversions (EFFECT, NOMINAL, RRI, PDURATION, DOLLARDE, DOLLARFR). **The five annuity functions are one equation rearranged**, so they share `annuity_factor` rather than repeating the algebra five times with five chances to flip a sign. **A zero rate is a limit, not an edge case to reject**: an interest-free loan is an ordinary thing to model and `((1+r)^n - 1)/r` is 0/0 there, so rejecting it would make PMT fail on the simplest case anyone tries. `RATE` has no closed form — Newton from the caller's guess, falling back to bisection, because the derivative is near zero around rate 0 and Newton alone diverges on exactly the ordinary nearly-interest-free case. Tests check **identities rather than recalled constants**: PV of PMT's stream is the loan back, FV of a fully repaid loan is zero, interest plus principal is the payment, NPV at the IRR is zero, every depreciation method exhausts exactly cost − salvage, and NOMINAL(EFFECT(r)) is r. **Two bugs found in my own first draft**: `DB` had three shadowed `rate` bindings with the first two dead, and `DOLLARDE`'s digit-shifting was invented rather than derived — the scale comes from the *digit count* of the denominator, so 1.02 at sixteenths is 1.125. **Still open**: the bond and coupon family (PRICE, YIELD, DURATION, COUP*, ACCRINT, ODD*), which need day-count conventions. 7 tests. 57.9% → 64.0% **The bond family is in**: the six `COUP*` schedule functions, then PRICE, YIELD, DURATION, MDURATION, ACCRINTM, PRICEDISC, YIELDDISC, PRICEMAT, YIELDMAT. Three things had to be right. **Coupons are counted backwards from maturity**, not forwards from issue — a bond's last payment lands on its maturity date, and stepping forwards puts every date a few days out whenever month lengths differ; the day-of-month is clamped per month so a bond maturing on the 31st pays on the 30th in a short month and returns to the 31st rather than drifting earlier for ever. **On a 30/360 basis a coupon period is 360/frequency days by definition**, so COUPDAYS is not measured from the calendar — measuring it makes it disagree with COUPDAYBS + COUPDAYSNC, which must sum to it, and there is a test across all five bases. **YIELD is solved against the very function PRICE uses**, so the two are exact inverses rather than approximately so; bisection rather than Newton, because the derivative degenerates near zero yield, which is exactly where a bond priced at par sits. Coverage 80.9% → 85.1%. Then VDB, ACCRINT, AMORLINC and AMORDEGRC. **VDB's switch to straight line is the whole point of it** — declining balance never reaches the salvage value, so an asset depreciated purely that way is still on the books at the end of its life; `no_switch` exists for the jurisdictions that require exactly that. **AMORLINC is linear on the original cost and AMORDEGRC degressive on the book** — the first implementation applied the rate to the declining book in both, which quietly turned the linear one into a second copy of the degressive one; the test caught it because a full period came out at 340.2 instead of 360. Two test-data errors of my own alongside it: VDB over five years at factor 2 never switches at all (the crossover needs a longer life), and DDB is already clamped at salvage so the lifetime total is the same either way — both premises were wrong, not the code. **The odd-period bonds close the category**: ODDFPRICE, ODDFYIELD, ODDLPRICE, ODDLYIELD. These are the awkward corner of bond maths and I cannot check them against Excel from here, so the tests assert the **property that pins them down** instead of quoting numbers: a bond whose odd period is actually regular is an ordinary bond, so a coupon equal to its yield must price at par whatever the schedule says. Each yield is solved against its own price function, so the pairs invert exactly. |
| FN-06 | Engineering | 39 | ✅ | Base conversion (all nine BIN/OCT/DEC/HEX pairs), the bit operations, DELTA, GESTEP, ERF and ERFC. **The trap is two's complement**: a ten-digit value with its top digit set is *negative*, so `BIN2DEC("1111111111")` is -1, not 1023. Parsing unsigned gives a large positive number that looks entirely plausible, and it is the single most likely mistake in these functions — pinned for all three radices. A `places` argument narrower than the value is an error rather than a silent truncation that would change the number, and it is ignored for negatives, since padding a two's-complement form changes its value. The bit operations are bounded at 2^48, which `f64` holds exactly, so a result is never a rounded approximation of the bits asked for. ERF/ERFC are checked against the identity `ERF(x) + ERFC(x) = 1` rather than recalled constants — that holds whatever the approximation's accuracy. **Still open**: complex numbers (the IM* family), Bessel and CONVERT. 4 tests. 53.4% → 57.9% **Complex numbers added**: the whole IM* family. They are *text* in a spreadsheet — `"3+4i"` — not a value type, so every function parses and re-formats, and **the suffix travels with the value**: a workbook written in `j` must not come back in `i`, so the first argument's suffix wins a fold. Two parsing details that a straightforward split gets wrong: `"3+i"` is 3+1i, since a bare sign is a coefficient of one; and the split must skip an exponent's sign, or `"1e-3+2i"` breaks at the exponent and parses as nonsense. Tested against identities — |3+4i| = 5, e^(iπ) = −1, √z² = z, (z·w)/w = z — rather than recalled constants. 3 tests. 64.0% → 69.1% **The Bessel family is in**: BESSELI, BESSELJ, BESSELK, BESSELY, as series rather than tables — a table is an approximation with an arbitrary cut-off instead of a stated error. Y and K diverge at zero and are undefined below it, which is `#NUM!` rather than an infinity: a spreadsheet showing `1E+308` for an undefined value is worse than one that says so. **Two of the test's reference constants were wrong when first written** — J₂(2.5) and K₁(1.5), mistyped in the seventh and ninth decimal — and the failure was the test, not the code; they were re-derived from published tables and from a separately written implementation of the same series before being changed. **CONVERT** goes through one base unit per category, so a conversion is a division: a pairwise table of eighty units is six thousand entries and every one a chance to be wrong. Units from different categories are `#N/A` — kilograms into metres is not a small error, it is a question with no answer. An exact unit name beats a prefixed reading, or `m` becomes milli-something and `T` becomes tera. Temperature is the one family a factor cannot express, because the scales have different zeros; there is a test that the Celsius and Fahrenheit scales cross at −40. |
...
## Functions that cannot be implemented here, and why

Named rather than left in a "missing" count, because the difference between
*not done* and *not possible* is the whole point of an honest audit.
