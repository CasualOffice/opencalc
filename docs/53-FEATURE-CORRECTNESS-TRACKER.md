...
| FC-20 | A filter covers the table, not the whole used range | P1 | ✅ | Pressing the toolbar filter with one cell selected filtered `A1` to the last used cell anywhere on the sheet, so **a column blank in the header row still got a filter button** — the reported "filter icon appears in an empty column". Same root cause as FC-17: the used region is a bounding box over the whole sheet, not the table you are in. It now uses `session_block_bounds` as well, falling back to the used region only when the cursor is on a blank cell and there is no block to take. |
