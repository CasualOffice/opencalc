...
| Data validation | ● | ● | ● | — | ◐ | All eight `type`s with operator, operands, `allowBlank` and message wording. Enforced in the editor for every kind except `custom`, which needs the formula engine |
...
| Theme colours (slot + tint linkage) | ● | ● | ● | ● | — | UX-FM6 |
| Threaded comments (replies, timestamps, resolved) | ● | ● | ● | ● | — | UX-P12 |
| CSV/TSV field typing (dates, leading zeros) | ● | ● | ● | ● | ● | UX-P15 |
