...
| UX-FM1 | Conditional formatting: fill-only cell rules; no color scales / data bars / top-bottom / above-avg / duplicates; no Manage Rules; no priority/stop-if-true; font/border effects | Format | 🟡 | **Colour scales (2 and 3 stop) and data bars done**, end to end: model, import (`colorScale`/`dataBar` with their `<color>` stops), export, panel, render. These are *range-relative* rather than per-cell predicates, so `session_cells` computes each rule's numeric span once per call and interpolates from it. **Remaining**: top/bottom N, above-average, duplicates, Manage Rules, priority/stop-if-true, font and border effects |
