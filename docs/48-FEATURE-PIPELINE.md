...
- ~~STALE-TRACKER FIXUP (do alongside M6): docs/45 and docs/46 claim 'full recalc only' and '8 functions' — both verified false (incremental recalc shipped; ~57 functions exist). Update CP-073/CP-080 so roadmap sizing isn't wrong, and reflect M6-3 as the perf (not correctness) work item.~~ **Overtaken:** `docs/45` and `docs/46` were deleted on 2026-08-18 for exactly the staleness this bullet describes, and `CP-073`/`CP-080` went with them — their tombstone rows are in [00](00-README.md). The "~57 functions" figure was wrong too, in the other direction; see `DOC-038` for why no count is published any more. M6-3 is done: the persistent graph is `PERF-04`, its range-edge indexing `PERF-06`.
