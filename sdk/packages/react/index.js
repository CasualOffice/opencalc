...
//
// Written with `createElement` rather than JSX so the package ships as plain
// JavaScript: no build step here, and no assumption about the JSX runtime,
// TypeScript settings or bundler in whatever project installs it.
...
export const OpenCalcSheet = forwardRef(function OpenCalcSheet(
  { engine, ui, onReady, onCellsChanged, onSelectionChanged, style, className, ...rest },
  ref,
) {
  ...
  // `class`, not `className`: this is a custom element, not a React DOM
  // component, so React sets the attribute verbatim.
  return createElement("opencalc-sheet", { ref: host, style, class: className, ...rest });
});
