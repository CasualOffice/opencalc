...
| 5 — Collaboration / web | operation model for shared editing | ⬜ decided, unbuilt: **server-mediated operational transform, not a CRDT** ([ADR-011](docs/08-ADR-REGISTER.md), [docs/56](docs/56-COLLABORATION-CONCURRENCY-DESIGN.md)), state as a snapshot plus the ops since it. One editor needs no server. Explicitly **not** part of the embeddable SDK, which is single-user |
