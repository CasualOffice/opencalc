...
export function drawChartFrame(ch, x, y, w, h) {
  ctx.fillStyle = colors.bg;
...
export function legendBox(ch, series, x, y, w, h, plot) {
  ctx.font = "10px " + (colors.uiFont || "system-ui, sans-serif");
...
export function drawLegend(series, box) {
...
export function drawAxes(plot, lo, hi) {
...
export function drawBarChart(ch, series, plot, kind) {
...
export function drawLineChart(ch, series, plot, kind) {
  const { lo, hi } = valueExtent(series);
  drawAxes(plot, lo, hi);
  const cols = seriesColors(series.length);
  const points = Math.max(...series.map((s) => s.values.length));
...
export function drawPie(ch, series, plot) {
...
export function drawCategoryLabels(ch, plot, points, kind) {
  const cats = ch.cats || [];
