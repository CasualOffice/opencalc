...
///
/// **Generated from the design note rather than transcribed from it.** §3 is a
/// hundred and forty-seven rows across eight tables, and a hand-copy of that is
/// a hundred and forty-seven chances to put Merge & Center in the wrong group.
/// The note is the source; this is its projection. When §3 changes, regenerate.
///
...
export const ASSIGN = [
  { id: "home", label: "Home", groups: [
    ...
    { label: "Font", ord: 9, items: [["toolbar.font","med"], ["toolbar.size","med"], "toolbar.size-up", "toolbar.size-down", "toolbar.bold", "format.bold", "toolbar.italic", "format.italic", "toolbar.underline", "format.underline", "toolbar.strike", "format.strikethrough", "format.superscript", "format.subscript", "toolbar.border", "toolbar.fillcolor", "toolbar.fontcolor", "format.format-cells"] },
    { label: "Alignment", ord: 8, items: ["format.alignment.top", "format.alignment.justify-vertical", "toolbar.rotate", "toolbar.wrap", "format.text-overflow.wrap", "format.alignment.left", "format.alignment.fill-repeat-text", "toolbar.indent-less", "toolbar.indent-more", "toolbar.merge", "format.format-cells"] },
    { label: "Number", ord: 7, items: [["toolbar.numfmt","med"], "toolbar.numfmt-label", "toolbar.currency", "toolbar.percent", "toolbar.comma", "toolbar.inc-dec", "toolbar.dec-dec", "format.format-cells"] },
    ...
    { label: "Cells", ord: 4, items: [["insert.rows-above","large"], "insert.cells", "insert.rows-below", "insert.sheet", ["insert.delete-rows","large"], "insert.delete-cells", "toolbar.delete-sheet", ["home.cells.format","large"], "format.row-height", "format.column-width", "format.autofit-row", "format.autofit-column", "data.hide-rows", "data.hide-columns", "data.unhide-rows-columns-in-selection", "data.unhide-all-rows-and-columns", "sheet.rename", "sheet.duplicate", "sheet.tab-color", "sheet.hide", "sheet.unhide", "format.format-cells"] },
  ...
  { id: "formulas", label: "Formulas", groups: [
    { label: "Function Library", ord: 6, items: [["formulas.insert-function","large"], "fx-insert"] },
  ...
  { id: "data", label: "Data", groups: [
    ...
    { label: "Sort & Filter", ord: 8, items: ["data.sort-range.a-z", ["data.sort-range.custom-sort","large"], ["toolbar.filter","large"], "data.filter", ["data.clear-all-filters","med"]] },
  ...
  { id: "review", label: "Review", groups: [
    ...
    { label: "Comments", ord: 6, items: [["insert.note","large"], ["insert.note.show","large"], ["edit.select-commented","med"]] },
  ...
  { id: "view", label: "View", groups: [
    ...
    { label: "Show", ord: 5, items: ["view.gridlines", "view.cell-markings", "view.zero-values"] },
